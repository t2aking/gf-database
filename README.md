...
同じ画面の「登録済みデータのCSVを準備」で、ローカルDBの全カタログ項目と所持情報を既存の11列のUTF-8 CSVに書き出せます。検索条件や一覧の表示件数制限は適用されません。1ファイルが1,000データ行または1MiBを超えないよう自動分割されるため、表示されたファイルを**すべて**ダウンロードしてください。登録項目が0件の場合はファイルを作りません。CSVには実データが入るので、リポジトリ外などGit管理外に保存してください。

書き出す値は種類別`details`、所持状態、数量、解放段階、覚醒、メモを含みます。出典情報とカタログの`metadata`はCSVの対象外です。既存項目への再取り込みでは保持されますが、空のDBへCSVだけを取り込んでも復元できません。全データの移行・復元には[バックアップと復元](#バックアップと復元)を使ってください。

編集するときは各CSVをUTF-8のまま扱い、表計算ソフトでは列を**文字列**として読み込んでください。書き出し時には先頭が `=`、`+`、`-`、`@` の文字列へ数式解釈を防ぐタブを付け、再取り込み時に除去します。表計算ソフトで保存し直すとタブや値が変わる場合があるため、プレビューで確認してください。編集後は各ファイルを順に「検証・プレビュー」し、意図した変更だけがあることを確認してから保存します。無変更のCSVでも既存項目は「更新」と表示されますが、名称・種類別詳細・所持情報に意図しない変更がないか確認してください。CSVには名前変更や削除の操作は含まれません。
...
本リポジトリで作成したソースコードと付属ドキュメントは [MIT License](LICENSE) で公開します。著作権表示は `Copyright (c) 2026 Tatsuaki Morioka` です。再利用・改変・再配布を簡潔な条件で認めるため、MIT Licenseを採用しました。

このライセンスは、実ゲームデータ、ゲームの画像・ロゴ・商標、攻略記事などの第三者コンテンツには適用されません。それらの利用権を本リポジトリが付与するものではありません。利用者がローカルDBに登録したデータも、このリポジトリのコードライセンスの対象外です。
