...
編集するときは各CSVをUTF-8のまま扱い、表計算ソフトでは列を**文字列**として読み込んでください。先頭が `=`、`+`、`-`、`@` の値は数式として解釈されることがあります。自動型変換による値の変化も確認してください。編集後は各ファイルを順に「検証・プレビュー」し、意図した変更だけがあることを確認してから保存します。無変更のCSVでも既存項目は「更新」と表示されますが、名称・種類別詳細・所持情報に意図しない変更がないか確認してください。CSVには名前変更や削除の操作は含まれません。
